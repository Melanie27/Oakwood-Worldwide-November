						<section class="two container">
							<div class="row heading">
								<section class="col-lg-12">
									<div class="container">
										<h1>News</h1>
									</div>
								</section>		
							</div><!--heading-->
							
							<div class="row over hidden-xs container">
								<section class="col-lg-12 vert">
										<div class="row">
											<section class="col-lg-4 hidden-md hidden-sm hidden-xs thirds">
												<img src="images/news/cover-story-tb-01.jpg" alt="cover-story-tb-01" class="img-responsive">
											</section> <!--end one third-->
											<section class="col-lg-4 col-md-6 col-sm-6 hidden-xs thirds blurb">
												<h2>Story Title Lorem Ipsum Dolore Sit Amet</h2>
												<P><span class="city">LOS ANGELES</span> - July 31, 2013 - Sed auctor neque eu tellus rhoncus ut eleifend nibh porttitor. Ut in nulla enim. Phasellus molestie magna non est bibendum non venenatis nisl tempor sppo suere lorem ipsum.</P>
												<p class="full-story"><a href="#">Full Story</a></p>
												<p>More from <span class="tags"><a href="#">Awards</a></span>, <a href="#">Oakwood Worldwide</a></p>
											</section> <!--end two thirds-->
											<section class="col-lg-4 col-md-6 col-sm-6 hidden-xs thirds teasers">
												<?php include "_/components/php/news-thumbs.php"; ?>	
										</section> <!--end three thirds-->	
									</div><!--end nested row-->	
								</section>	
							</div><!--heading-->	
						</section> <!--two container-->	